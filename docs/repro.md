...
> [!NOTE] 
> 1. The results in the next section display only some important subsets of metrics for each benchmark. For examples of more available evaluation metrics available: see `muse*/*_EVAL.json`, `tofu*/evals*/*_EVAL.json` files on the [HuggingFace space](https://huggingface.co/datasets/open-unlearning/eval).
